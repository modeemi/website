...
def remind():
    ...
    if settings.MODE_DRY_RUN:
        logger.info('Would remind: %s', list(memberships.values_list("user__username", flat=True)))
        return
...
def deactivate():
    ...
    if settings.MODE_DRY_RUN:
        logger.info('Would deactivate: %s', list(memberships.values_list("user__username", flat=True)))
        return
