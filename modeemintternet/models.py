...
class MembershipFee(models.Model):
    year = models.PositiveIntegerField(
        primary_key=True,
        unique=True,
        validators=[MinValueValidator(1975), MaxValueValidator(now().year + 1)],
        verbose_name="Vuosi",
    )
...
class Membership(models.Model):
    user = models.OneToOneField(
        settings.AUTH_USER_MODEL, on_delete=models.PROTECT, verbose_name="Käyttäjä"
    )
    ...
    municipality = models.CharField(
        max_length=128, blank=True, default="", verbose_name="Kotipaikka"
    )
    key_engineering = models.BooleanField(
        default=False, verbose_name="Konehuoneen kova-avain"
    )
    key_physical = models.BooleanField(
        default=False, verbose_name="Kerhohuoneen kova-avain"
    )
    key_virtual = models.BooleanField(
...
class Application(models.Model):
    class Shell:
        ...
        CHOICES = ((SH, SH), (BASH, BASH), (ZSH, ZSH), (TCSH, TCSH), (FALSE, FALSE))
